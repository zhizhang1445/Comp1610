...
int main(int argc, char* argv[])
{
    ...
    if (argc != 3) {
        std::cerr << "Usage:\n"
                  << "  ./analysewave_omp NETCDFFILENAME OUTPUTFILENAME\n"
	...
	// accumulate energy, sum, min and mean of the wave
        ...
        #pragma omp parallel default(none) shared(snapshot, snapshotprev, dx, dt, c) reduction(+:energy, sum) reduction(max : max) reduction(min : min)
